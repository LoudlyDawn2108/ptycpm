...
> **Ghi chú thiết kế – FEAT 2.4:** FEAT 2.4 mô tả ràng buộc phân quyền theo vai trò cho tài khoản người dùng. Các vai trò chức năng được hệ thống định nghĩa sẵn: Quản trị viên, Phòng TCCB, Phòng TCKT, Cán bộ nhân sự (Cán bộ/Giảng viên/Nhân viên). Mỗi vai trò quy định tập quyền truy cập các chức năng hệ thống khác nhau. FEAT này được hiện thực hóa bởi SUPL-DC03 nên không tách thành UC riêng; việc gán vai trò được tích hợp trong luồng thêm/sửa tài khoản (UC 4.5, UC 4.6).

> **Ghi chú thiết kế – FEAT 1.5:** FEAT 1.5 mô tả hình thức cung cấp hệ thống dưới dạng ứng dụng web SPA + RESTful API, bắt nguồn từ STRQ 14 qua kỹ thuật *Thêm chi tiết*. FEAT này được hiện thực hóa bởi SUPL-DC01 nên không tách thành UC riêng.

> **Ghi chú thiết kế – FEAT 3.1:** FEAT 3.1 mô tả ràng buộc cơ cấu tổ chức phân cấp dạng cây cha-con với gốc duy nhất, bắt nguồn từ STRQ 15 qua kỹ thuật *Thêm chi tiết*. FEAT này được hiện thực hóa bởi SUPL-DC02 nên không tách thành UC riêng.
...
- UC 4.10: Cập nhật trạng thái cho đơn vị tổ chức nhân sự
...
| FEAT 3.4 | UC 4.10 (Cập nhật trạng thái cho đơn vị tổ chức nhân sự) |
...
| Điều kiện tiên quyết (Precondition) | Quản trị viên đã đăng nhập hệ thống. Tài khoản cần chỉnh sửa tồn tại và còn hiệu lực trong hệ thống. |
...
### 4.10. Use Case: Cập nhật trạng thái cho đơn vị tổ chức nhân sự

|  |  |
| --- | --- |
| **Tên use case** | **Cập nhật trạng thái cho đơn vị tổ chức nhân sự** |
| Tác nhân chính | Quản trị viên, Phòng TCCB |
| Mục đích (mô tả) | Cho phép Quản trị viên hoặc Phòng TCCB cập nhật trạng thái hoạt động của đơn vị tổ chức (Giải thể hoặc Sáp nhập). |
| Mức độ ưu tiên (Priority) | Bắt buộc |
| Điều kiện kích hoạt (Trigger) | Người dùng chọn chức năng "Cập nhật trạng thái" đối với một đơn vị tại menu "Cơ cấu tổ chức". |
| Điều kiện tiên quyết (Precondition) | Người dùng đã đăng nhập hệ thống với vai trò Quản trị viên hoặc Phòng TCCB. Đơn vị tổ chức cần cập nhật đã tồn tại và đang ở trạng thái "Đang hoạt động". |
| Điều kiện thành công (Post-condition) | Nếu giải thể: trạng thái đơn vị là "Giải thể"; dữ liệu nhân sự và hợp đồng được cập nhật theo quy định nghiệp vụ. Nếu sáp nhập: trạng thái đơn vị là "Sáp nhập"; nhân sự và đơn vị con được chuyển sang đơn vị nhận sáp nhập. Lịch sử thay đổi được ghi nhận đầy đủ. |
| Điều kiện thất bại | Việc cập nhật trạng thái không được thực hiện do vi phạm ràng buộc nghiệp vụ hoặc dữ liệu không hợp lệ. |
| Luồng sự kiện chính (Basic Flow) | 1. Hệ thống hiển thị sơ đồ cây cơ cấu tổ chức hiện tại. 2. Người dùng chọn một đơn vị đang ở trạng thái "Đang hoạt động". 3. Người dùng chọn chức năng "Cập nhật trạng thái đơn vị". 4. Hệ thống hiển thị biểu mẫu cập nhật, đồng thời hiển thị cảnh báo danh sách các đơn vị con trực thuộc (nếu có). 5. Người dùng chọn loại sự kiện: **Giải thể**. 6. Người dùng nhập thông tin bắt buộc: Ngày hiệu lực (ngày giải thể); Quyết định (Số quyết định, Ngày quyết định, File đính kèm); Lý do (Giải thể / Tái cơ cấu / Khác). 7. Xử lý đơn vị con (nếu có): Người dùng chọn phương án — Tùy chọn A: Giải thể toàn bộ đơn vị con; Tùy chọn B: Điều chuyển đơn vị con sang đơn vị cha khác (chọn đơn vị cha mới từ danh sách). 8. Người dùng nhấn "Lưu xác nhận". 9. Hệ thống kiểm tra tính hợp lệ dữ liệu đầu vào. 10. Hệ thống thực hiện cập nhật: cập nhật trạng thái đơn vị thành "Giải thể"; xử lý đơn vị con theo phương án đã chọn ở bước 7; cập nhật nhân sự thuộc đơn vị: tất cả hợp đồng đang hiệu lực chuyển thành "Hết hiệu lực", trạng thái hợp đồng nhân sự chuyển thành "Chưa hợp đồng", trạng thái làm việc chuyển thành "Đang chờ xét", xóa thông tin đơn vị công tác hiện tại. 11. Hệ thống lưu lịch sử thay đổi và hiển thị thông báo "Cập nhật thành công". |
| Luồng sự kiện thay thế (Alternative Flow) | **A1: Sáp nhập đơn vị** A1.1. Tại bước 5, người dùng chọn loại sự kiện **Sáp nhập** thay vì Giải thể. A1.2. Người dùng nhập thông tin bắt buộc: Ngày hiệu lực, Quyết định, Lý do và chọn Đơn vị nhận sáp nhập từ danh sách đơn vị đang hoạt động. A1.3. Hệ thống tự động thiết lập chuyển toàn bộ đơn vị con trực thuộc sang làm đơn vị con của Đơn vị nhận sáp nhập. A1.4. Người dùng nhấn "Lưu xác nhận". A1.5. Hệ thống kiểm tra tính hợp lệ dữ liệu. A1.6. Hệ thống thực hiện cập nhật: cập nhật trạng thái đơn vị thành "Sáp nhập"; cập nhật đơn vị quản lý cấp trên của đơn vị con sang Đơn vị nhận sáp nhập; chuyển toàn bộ nhân sự sang Đơn vị nhận sáp nhập (trạng thái hợp đồng giữ nguyên, trạng thái làm việc giữ nguyên "Đang công tác", thông tin đơn vị mới được cập nhật). A1.7. Hệ thống lưu lịch sử thay đổi và hiển thị thông báo "Cập nhật thành công". A1.8. Use case kết thúc. |
| Luồng sự kiện ngoại lệ (Exception Flow) | **E1: Đơn vị không hợp lệ để cập nhật trạng thái** E1.1. Tại bước 2, nếu đơn vị đang ở trạng thái "Giải thể" hoặc "Sáp nhập". E1.2. Hệ thống vô hiệu hóa chức năng và hiển thị thông báo "Đơn vị này không còn hoạt động". E1.3. Use case kết thúc. **E2: Dữ liệu không hợp lệ** E2.1. Tại bước 9 hoặc A1.5, nếu thiếu thông tin bắt buộc, ngày hiệu lực sai định dạng hoặc chưa đính kèm file Quyết định. E2.2. Hệ thống chặn thao tác lưu, đánh dấu các trường bị lỗi và yêu cầu nhập lại. E2.3. Luồng quay lại bước 6 hoặc A1.2. **E3: Chưa xử lý đơn vị con khi Giải thể** E3.1. Tại bước 7, nếu đơn vị có đơn vị con nhưng người dùng chọn Tùy chọn B mà không chọn Đơn vị cha mới. E3.2. Hệ thống hiển thị cảnh báo "Vui lòng chọn đơn vị quản lý mới cho các đơn vị trực thuộc trước khi giải thể." E3.3. Luồng quay lại bước 7. **E4: Ràng buộc chức vụ quản lý** E4.1. Tại bước 9 hoặc A1.5, nếu đơn vị đang có nhân sự giữ chức vụ quản lý (ví dụ: Trưởng phòng, Trưởng khoa). E4.2. Hệ thống chặn thao tác và hiển thị thông báo "Vui lòng thực hiện bãi nhiệm chức vụ quản lý của đơn vị trước khi thực hiện giải thể/sáp nhập." E4.3. Use case kết thúc. |
...
| **SUPL-DC02** | Cơ cấu tổ chức dạng cây | Hệ thống phải biểu diễn cơ cấu tổ chức dưới dạng cây cha-con với một nút gốc duy nhất là Trường Đại học Thủy Lợi. | FEAT 3.1 (Phúc) | **M** |
